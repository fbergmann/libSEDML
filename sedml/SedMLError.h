 ...
 *
 * When a libSedML operation on SedML content results in an error, or when
 * there is something wrong with the SedML content, the problems are
 * reported as SedMLError objects.  These are generally stored in an
 * SedMLErrorLog object; this log object, in turn, is kept in the
 * SedMLDocument object containing the SedML content.  Applications can
 * obtain the list of logged errors using SedMLDocument::getErrorLog() and
 * then use the methods provided by SedMLErrorLog to access individual
 * SedMLError objects.  (Note that despite the word "error" in the name,
 * SedMLError objects are used to represent not only "true" errors, but also
 * warnings and some informational diagnostics.  The name is a historical
 * hold-over from early versions of libSedML, in which the object really was
 * only used to report errors.)
 *
 * @if clike
 * Each SedMLError object instance has an identification number that
 ...
 * <a class="el" href="#SedMLErrorCode_t"> SedMLErrorCode_t</a> (described <a
 * class="el" href="#SedMLErrorCode_t"> below</a>) or @link
 ...
 * all the SedML validation rule numbers listed in the appendices of the
 * SedML specification documents, as well as some additional
 * libSedML-specific error codes.
 * @endif@if java
 * Each SedMLError object instance has an identification number that
 * identifies the nature of the problem.  This "error id" number will be up
 ...
 * SedML error identifiers <a class="el" href="#SedMLErrorCode_t"> (described
 ...
 * former set of constants includes all the SedML validation rule numbers
 * listed in the appendices of the SedML specification documents, as well as
 * some additional libSedML-specific error codes.
 * @endif@if python
 * Each SedMLError object instance has an identification number that
 * identifies the nature of the problem.  This "error id" number will be up
 * to five digits long, and it will come from one
 * of two sets of static integer constants defined in
 * the interface class @link libsbml libsbml@endlink: either the SedML
 * error identifiers <a
 * class="el" href="#SedMLErrorCode_t"> (described below)</a> or the XML
 * error identifiers (described in the documentation for the class XMLError).
 * The former set of constants
 * includes all the SedML validation rule numbers listed in the appendices
 * of the SedML specification documents, as well as some additional
 * libSedML-specific error codes.
 * @endif@~
 * 
 * Error codes are useful mainly for software.  For human readers,
 * SedMLError also includes text messages that describe the nature of a
 * given problem.  The messages can be accessed using
 * SedMLError::getShortMessage() and SedMLError::getMessage().  The former
 * provides a brief one-line description of the issue, while
 * SedMLError::getMessage() provides a more detailed text, including (if
 * appropriate) references to sections of the SedML specifications where
 * relevant topics are discussed.  These text strings are suitable for
 * displaying to human users.
 *
 * @if clike
 * An SedMLError object also contains a category code; its value may be
 * retrieved using the method SedMLError::getCategory().  Category values
 * are drawn from the enumeration <a class="el"
 * href="#SedMLErrorCategory_t">SedMLErrorCategory_t</a> described below.
 * Categories are used to partition errors into distinct conceptual groups.
 * This is principally used by the libSedML validation system to group
 * classes of validation checks.  For example, 
 * @link SedMLErrorCategory_t#LIBSEDML_CAT_IDENTIFIER_CONSISTENCY LIBSEDML_CAT_IDENTIFIER_CONSISTENCY@endlink
 * is the category for tests that check identifier consistency;
 * @link SedMLErrorCategory_t#LIBSEDML_CAT_MATHML_CONSISTENCY LIBSEDML_CAT_MATHML_CONSISTENCY@endlink
 ...
 * distinct conceptual groups.  This is principally used by the libSedML
 ...
 * @endif@~
 *
 * In addition, SedMLError also has a severity code.  Its value may be
 * retrieved using the method SedMLError::getSeverity().  The possible
 ...
 * is. 
 *
 * Finally, SedMLError records the line and column near where the problem
 * occurred in the SedML content.  The values may be retrieved using the
 * methods SedMLError::getLine() and SedMLError::getColumn().  We say "near",
 ...
 * altogether.  If this occurs, libSedML sets the line and/or column number
 * in the SedMLError object to the the value of the maximum unsigned long
 * integer representable on the platform where libSedML is running.  (This
 * is equal to the constant named <code>ULONG_MAX</code> in C and C++.)
 * The probability that a true line or column number in an SedML model would
 ...
 * line/column number could be provided by libSedML in that situation.
 *
 * @if clike
 * <h3><a class="anchor" name="SedMLErrorCode_t">SedMLErrorCode_t</a></h3>
 *
 * SedMLErrorCode_t is an enumeration of all SedML-level error, warning and
 * informational diagnostic codes.  Every SedMLError object has an error
 ...
 * an SedML issue per se.  The values of SedMLErrorCode_t are distinguished
 ...
 * lower.  The method SedMLError::getErrorId() returns the error code of a
 * given SedMLError object instance.
 * 
 * The following is a table of the symbolic names of SedMLErrorCode_t values
 * and the meaning of each code.  In this table, the right-hand columns
 * titled "L1V1", "L1V2", etc. refer to Levels and Versions of the SedML
 * specifications, and the entries in each column refer to whether the
 * severity of the condition in that particular Level+Version of SedML.
 * The codes stand for the following:
 * 
 * @endif@if java <h3><a class="anchor" 
 * name="SedMLErrorCode_t">Error codes associated with SedMLError objects</a></h3>
 *
 * The error and warning codes returned by libSedML are listed in the table
 * below.  The method SedMLError::getErrorId() returns the error code of a
 * given SedMLError object instance.  In the libSedML Java language
 ...
 * enumerations in Java prior to JDK 1.5.  Future versions of libSedML may
 ...
 * to Levels and Versions of the SedML specifications, and the entries in
 * each column refer to whether the severity of the condition in that
 * particular Level+Version of SedML.  The codes stand for the following:
 *
 ...
 * The error and warning codes returned by libSedML are listed in the table
 * below.  The method SedMLError::getErrorId() returns the error code of a
 * given SedMLError object instance.  In the libSedML Python language
 * interface, these error identifiers are currently
 * implemented as static integer constants defined in the interface class
 ...
 * SedMLError::getShortMessage() method on a given SedMLError object.  A
 * longer and (hopefully) clearer explanation of the issue is returned by
 * SedMLError::getMessage().
 *
 * @htmlinclude sbmlerror-table.html
 * 
 * @if clike <h3><a class="anchor" name="SedMLErrorCategory_t">SedMLErrorCategory_t</a></h3>
 *
 * SedMLErrorCategory_t is an enumeration of category codes for SedMLError
 * diagnostics.  The category can be retrieved from an SedMLError object
 * using the method SedMLError::getCategory().  These enumeration values are
 * distinct from (and in addition to) the @link
 * XMLError::XMLErrorCategory_t XMLErrorCategory_t @endlink codes used by
 * the parent XMLError object.  User programs receiving an SedMLError object
 * can use this distinction to check whether the error represents a
 * low-level XML problem or an SedML problem.
 *
 * The following table lists each possible value and a brief description of
 * its meaning.
 * 
 * @endif@if python <h3><a class="anchor" name="SedMLErrorCategory_t">Category codes associated with SedMLError objects</a></h3>
 *
 * As discussed above, each SedMLError object contains a value for a
 * category identifier, describing the type of issue that the SedMLError
 * object represents.  The category can be retrieved from an SedMLError
 * object using the method SedMLError::getCategory().  The following table
 * lists each possible value and a brief description of its meaning.
 *
 * As is the case with the error codes, in the libSedML Python language
 ...
 * 
 * @endif@if java <h3><a class="anchor"
 * name="SedMLErrorCategory_t">Category codes associated with SedMLError objects</a></h3>
 * 
 * As discussed above, each SedMLError object contains a value for a
 * category identifier, describing the type of issue that the SedMLError
 * object represents.  The category can be retrieved from an SedMLError
 ...
 * lists each possible value and a brief description of its meaning.
 * 
 * As is the case with the error codes, in the libSedML Java language
 * interface, the category identifiers are currently implemented as static
 * integer constants defined in the interface class
 ...
 * lists each possible value and a brief description of its meaning.
 * 
 * As is the case with the error codes, in the libSedML C# language
 * interface, the category identifiers are currently implemented as static
 * integer constants defined in the interface class
 ...
 * of SedML to another.  This particular category applies to errors
 * encountered while trying to convert a model from SedML Level&nbsp;2 to SedML
 * Level&nbsp;1.</td></tr> 
 ...
 * that can only occur during attempted translation from one Level/Version
 * of SedML to another.  This particular category applies to errors
 * encountered while trying to convert a model to SedML Level&nbsp;2
 * Version&nbsp;1.</td></tr> 
 ...
 * errors that can occur while validating general SedML constructs.  With
 * respect to the SedML specification, these concern failures in applying
 ...
 * With respect to the SedML specification, these concern failures in
 ...
 * quantities in a model.  With respect to the SedML specification, these
 ...
 * therefore violating a tenet of proper SedML.  With respect to the SedML
 * specification, this is validation rule #10601 in the SedML Level&nbsp;2 Versions&nbsp;2&ndash;4
 * and Level&nbsp;3 Version&nbsp;1 specifications.</td></tr> 
 ...
 * warnings about recommended good practices involving SedML and
 * computational modeling.  (These are tests performed by libSedML and do
 * not have equivalent SedML validation rules.)</td></tr> 
 ...
 * errors that can occur while validating libSedML's internal representation
 * of SedML constructs. (These are tests performed by libSedML and do
 * not have equivalent SedML validation rules.)</td></tr> 
 ...
 * that can only occur during attempted translation from one Level/Version
 * of SedML to another.  This particular category applies to errors
 * encountered while trying to convert a model to SedML Level&nbsp;3
 * Version&nbsp;1.</td></tr> 
 *
 * </table>
 * </center>
 * 
 * @if clike
 * <h3><a class="anchor" name="SedMLErrorSeverity_t">SedMLErrorSeverity_t</a></h3>
 *
 * This is an enumeration of severity codes for SedMLError diagnostics.
 * User programs receiving an SedMLError object can use this distinction to
 * check whether the error represents a low-level XML problem or an SedML
 * problem.
 * 
 * In libSedML version @htmlinclude libsbml-version.html
 * there are no additional severity codes in SedMLErrorSeverity_t beyond
 * those defined in @link XMLError::XMLErrorSeverity_t XMLErrorSeverity_t@endlink.
 * 
 * <hr>
 * @endif@if java <h3><a class="anchor"
 * name="SedMLErrorSeverity_t">Severity codes associated with SedMLError
 * objects</h3>
 *
 * In libSedML version @htmlinclude libsbml-version.html
...
/**
 * Codes for all SedML-level errors and warnings.
 *
 ...
 * programs may wish to check whether a given SedMLError object's error
 * identifier is actually from SedMLErrorCode_t or XMLError::XMLErrorCode_t.
...
, NotSchemaConformant                   = 10103 /*!< Document does not conform to the SedML XML schema. */
...
, SedMLNamespaceInAnnotation             = 10403 /*!< The SedML XML namespace cannot be used in an Annotation object. */
...
, InvalidNamespaceOnSedML                = 20101 /*!< Invalid XML namespace for the SedML container element. */
...
, PackageNSMustMatch                    = 20104 /*!< Inconsistent or invalid SedML Level/Version for the package namespace declaration. */
...
, AllowedAttributesOnSedML               = 20108 /*!< Invalid attribute found on the SedML container element. */
, L3PackageOnLowerSedML                  = 20109 /*!< An L3 package ns found on the SedML container element.. */
...
, OffsetNoLongerValid                   = 20411 /*!< Unit attribute 'offset' is not supported in this Level+Version of SedML. */
, CelsiusNoLongerValid                  = 20412 /*!< Unit name 'Celsius' is not defined in this Level+Version of SedML. */
...
, SpatialSizeUnitsRemoved               = 20615 /*!< Attribute 'spatialSizeUnits' is not supported in this Level+Version of SedML. */
...
, SubsUnitsNoLongerValid                = 21125 /*!< Attribute 'substanceUnits' is not supported in this Level+Version of SedML. */
, TimeUnitsNoLongerValid                = 21126 /*!< Attribute 'timeUnits' is not supported in this Level+Version of SedML. */
...
, LibSedMLAdditionalCodesLowerBound      = 90000 /*!< Lower boundary of libSedML-specific diagnostic codes. */
, CannotConvertToL1V1                   = 90001 /*!< Cannot convert to SedML Level 1 Version 1. */
, NoEventsInL1                          = 91001 /*!< SedML Level 1 does not support events. */
, NoFunctionDefinitionsInL1             = 91002 /*!< SedML Level 1 does not support function definitions. */
, NoConstraintsInL1                     = 91003 /*!< SedML Level 1 does not support constraints. */
, NoInitialAssignmentsInL1              = 91004 /*!< SedML Level 1 does not support initial assignments. */
, NoSpeciesTypesInL1                    = 91005 /*!< SedML Level 1 does not support species types. */
, NoCompartmentTypeInL1                 = 91006 /*!< SedML Level 1 does not support compartment types. */
, NoNon3DCompartmentsInL1               = 91007 /*!< SedML Level 1 only supports three-dimensional compartments. */
, NoFancyStoichiometryMathInL1          = 91008 /*!< SedML Level 1 does not support non-integer nor non-rational stoichiometry formulas. */
, NoNonIntegerStoichiometryInL1         = 91009 /*!< SedML Level 1 does not support non-integer 'stoichiometry' attribute values. */
, NoUnitMultipliersOrOffsetsInL1        = 91010 /*!< SedML Level 1 does not support multipliers or offsets in unit definitions. */
, SpeciesCompartmentRequiredInL1        = 91011 /*!< In SedML Level 1, a value for 'compartment' is mandatory in species definitions. */
, NoSpeciesSpatialSizeUnitsInL1         = 91012 /*!< SedML Level 1 does not support species 'spatialSizeUnits' settings. */
, NoSBOTermsInL1                        = 91013 /*!< SedML Level 1 does not support the 'sboTerm' attribute. */
, StrictUnitsRequiredInL1               = 91014 /*!< SedML Level 1 requires strict unit consistency. */
, ConversionFactorNotInL1               = 91015 /*!< SedML Level 1 does not support the 'conversionFactor' attribute. */
, CompartmentNotOnL1Reaction            = 91016 /*!< SedML Level 1 does not support the 'compartment' attribute on Reaction objects. */
...
, HasOnlySubstanceUnitsNotinL1          = 91019 /*!< The concept of hasOnlySubstanceUnits was not available in SedML Level 1.. */
, AvogadroNotSupported                  = 91020 /*!< Avogadro not supported in Levels 2 and 1.. */
, NoConstraintsInL2v1                   = 92001 /*!< SedML Level 2 Version 1 does not support Constraint objects. */
, NoInitialAssignmentsInL2v1            = 92002 /*!< SedML Level 2 Version 1 does not support InitialAssignment objects. */
, NoSpeciesTypeInL2v1                   = 92003 /*!< SedML Level 2 Version 1 does not support SpeciesType objects. */
, NoCompartmentTypeInL2v1               = 92004 /*!< SedML Level 2 Version 1 does not support CompartmentType objects. */
, NoSBOTermsInL2v1                      = 92005 /*!< SedML Level 2 Version 1 does not support the 'sboTerm' attribute. */
, NoIdOnSpeciesReferenceInL2v1          = 92006 /*!< SedML Level 2 Version 1 does not support the 'id' attribute on SpeciesReference objects. */
, NoDelayedEventAssignmentInL2v1        = 92007 /*!< SedML Level 2 Version 1 does not support the 'useValuesFromTriggerTime' attribute. */
, StrictUnitsRequiredInL2v1             = 92008 /*!< SedML Level 2 Version 1 requires strict unit consistency. */
, IntegerSpatialDimensions              = 92009 /*!< SedML Level 2 Version 1 requires that compartments have spatial dimensions of 0-3. */
, StoichiometryMathNotYetSupported      = 92010 /*!< Conversion to StoichiometryMath objects not yet supported. */
, PriorityLostFromL3                    = 92011 /*!< SedML Level 2 Version 1 does not support priorities on Event objects. */
, NonPersistentNotSupported             = 92012 /*!< SedML Level 2 Version 1 does not support the 'persistent' attribute on Trigger objects. */
, InitialValueFalseEventNotSupported    = 92013 /*!< SedML Level 2 Version 1 does not support the 'initialValue' attribute on Trigger objects. */
, SBOTermNotUniversalInL2v2             = 93001 /*!< The 'sboTerm' attribute is invalid for this component in SedML Level 2 Version 2. */
, NoUnitOffsetInL2v2                    = 93002 /*!< This Level+Version of SedML does not support the 'offset' attribute on Unit objects. */
, NoKineticLawTimeUnitsInL2v2           = 93003 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on KineticLaw objects. */
, NoKineticLawSubstanceUnitsInL2v2      = 93004 /*!< This Level+Version of SedML does not support the 'substanceUnits' attribute on KineticLaw objects. */
, NoDelayedEventAssignmentInL2v2        = 93005 /*!< This Level+Version of SedML does not support the 'useValuesFromTriggerTime' attribute. */
, ModelSBOBranchChangedBeyondL2v2       = 93006 /*!< The allowable 'sboTerm' attribute values for Model objects differ for this SedML Level+Version. */
, StrictUnitsRequiredInL2v2             = 93007 /*!< SedML Level 2 Version 2 requires strict unit consistency. */
, StrictSBORequiredInL2v2               = 93008 /*!< SedML Level 2 Version 2 requires strict SBO term consistency. */
, DuplicateAnnotationInvalidInL2v2      = 93009 /*!< Duplicate top-level annotations are invalid in SedML Level 2 Version 2. */
, NoUnitOffsetInL2v3                    = 94001 /*!< This Level+Version of SedML does not support the 'offset' attribute on Unit objects. */
, NoKineticLawTimeUnitsInL2v3           = 94002 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on KineticLaw objects. */
, NoKineticLawSubstanceUnitsInL2v3      = 94003 /*!< This Level+Version of SedML does not support the 'substanceUnits' attribute on KineticLaw objects. */
, NoSpeciesSpatialSizeUnitsInL2v3       = 94004 /*!< This Level+Version of SedML does not support the 'spatialSizeUnit' attribute on Species objects. */
, NoEventTimeUnitsInL2v3                = 94005 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on Event objects. */
, NoDelayedEventAssignmentInL2v3        = 94006 /*!< This Level+Version of SedML does not support the 'useValuesFromTriggerTime' attribute. */
, ModelSBOBranchChangedBeyondL2v3       = 94007 /*!< The allowable 'sboTerm' attribute values for Model objects differ for this SedML Level+Version. */
, StrictUnitsRequiredInL2v3             = 94008 /*!< SedML Level 2 Version 3 requires strict unit consistency. */
, StrictSBORequiredInL2v3               = 94009 /*!< SedML Level 2 Version 3 requires strict SBO term consistency. */
, DuplicateAnnotationInvalidInL2v3      = 94010 /*!< Duplicate top-level annotations are invalid in SedML Level 2 Version 3. */
, NoUnitOffsetInL2v4                    = 95001 /*!< This Level+Version of SedML does not support the 'offset' attribute on Unit objects. */
, NoKineticLawTimeUnitsInL2v4           = 95002 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on KineticLaw objects. */
, NoKineticLawSubstanceUnitsInL2v4      = 95003 /*!< This Level+Version of SedML does not support the 'substanceUnits' attribute on KineticLaw objects. */
, NoSpeciesSpatialSizeUnitsInL2v4       = 95004 /*!< This Level+Version of SedML does not support the 'spatialSizeUnit' attribute on Species objects. */
, NoEventTimeUnitsInL2v4                = 95005 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on Event objects. */
, ModelSBOBranchChangedInL2v4           = 95006 /*!< The allowable 'sboTerm' attribute values for Model objects differ for this SedML Level+Version. */
, DuplicateAnnotationInvalidInL2v4      = 95007 /*!< Duplicate top-level annotations are invalid in SedML Level 2 Version 4. */
, NoSpeciesTypeInL3v1                   = 96001 /*!< SedML Level 3 Version 1 does not support SpeciesType objects. */
, NoCompartmentTypeInL3v1               = 96002 /*!< SedML Level 3 Version 1 does not support CompartmentType objects. */
, NoUnitOffsetInL3v1                    = 96003 /*!< This Level+Version of SedML does not support the 'offset' attribute on Unit objects. */
, NoKineticLawTimeUnitsInL3v1           = 96004 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on KineticLaw objects. */
, NoKineticLawSubstanceUnitsInL3v1      = 96005 /*!< This Level+Version of SedML does not support the 'substanceUnits' attribute on KineticLaw objects. */
, NoSpeciesSpatialSizeUnitsInL3v1       = 96006 /*!< This Level+Version of SedML does not support the 'spatialSizeUnit' attribute on Species objects. */
, NoEventTimeUnitsInL3v1                = 96007 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on Event objects. */
, ModelSBOBranchChangedInL3v1           = 96008 /*!< The allowable 'sboTerm' attribute values for Model objects differ for this SedML Level+Version. */
, DuplicateAnnotationInvalidInL3v1      = 96009 /*!< Duplicate top-level annotations are invalid in SedML Level 3 Version 1. */
, NoCompartmentOutsideInL3v1            = 96010 /*!< This Level+Version of SedML does not support the 'outside' attribute on Compartment objects. */
, NoStoichiometryMathInL3v1             = 96011 /*!< This Level+Version of SedML does not support the StoichiometryMath object. */
, InvalidSedMLLevelVersion               = 99101 /*!< Unknown Level+Version combination of SedML. */
, AnnotationNotesNotAllowedLevel1       = 99104 /*!< Annotation objects on the SedML container element are not permitted in SedML Level 1. */
, InvalidRuleOrdering                   = 99106 /*!< Invalid ordering of rules. */
, RequiredPackagePresent                = 99107 /*!< The SedML document requires an SedML Level 3 package unavailable in this software. */
, UnrequiredPackagePresent              = 99108 /*!< The SedML document uses an SedML Level 3 package unavailable in this software. */
...
, FormulaInLevel1KL                     = 99129 /*!< Only predefined functions are allowed in SedML Level 1 formulas. */
, L3SubstanceUnitsOnModel               = 99130 /*!< Invalid 'substanceUnits' attribute value. */
, TimeUnitsRemoved                      = 99206 /*!< This Level+Version of SedML does not support the 'timeUnits' attribute on Event objects. */
...
, IncorrectCompartmentSpatialDimensions = 99901 /*!< In SedML Level 1, only three-dimensional compartments are allowed. */
, CompartmentTypeNotValidAttribute      = 99902 /*!< CompartmentType objects are not available in this Level+Version of SedML. */
, ConstantNotValidAttribute             = 99903 /*!< This Level+Version of SedML does not support the 'constant' attribute on this component. */
, MetaIdNotValidAttribute               = 99904 /*!< Attribute 'metaid' is not available in SedML Level 1. */
, SBOTermNotValidAttributeBeforeL2V3    = 99905 /*!< The 'sboTerm' attribute is not available on this component before SedML Level 2 Version 3. */
, InvalidL1CompartmentUnits             = 99906 /*!< Invalid units for a compartment in SedML Level 1. */
, L1V1CompartmentVolumeReqd             = 99907 /*!< In SedML Level 1, a compartment's volume must be specified. */
, CompartmentTypeNotValidComponent      = 99908 /*!< CompartmentType objects are not available in this Level+Version of SedML. */
, ConstraintNotValidComponent           = 99909 /*!< Constraint objects are not available in this Level+Version of SedML. */
, EventNotValidComponent                = 99910 /*!< Event objects are not available in this Level+Version of SedML. */
, SBOTermNotValidAttributeBeforeL2V2    = 99911 /*!< The 'sboTerm' attribute is invalid for this component before Level 2 Version 2. */
, FuncDefNotValidComponent              = 99912 /*!< FunctionDefinition objects are not available in this Level+Version of SedML. */
, InitialAssignNotValidComponent        = 99913 /*!< InitialAssignment objects are not available in this Level+Version of SedML. */
, VariableNotValidAttribute             = 99914 /*!< Attribute 'variable' is not available on this component in this Level+Version of SedML. */
, UnitsNotValidAttribute                = 99915 /*!< Attribute 'units' is not available on this component in this Level+Version of SedML. */
, ConstantSpeciesNotValidAttribute      = 99916 /*!< Attribute 'constant' is not available on Species objects in SedML Level 1. */
, SpatialSizeUnitsNotValidAttribute     = 99917 /*!< Attribute 'spatialSizeUnits' is not available on Species objects in SedML Level 1. */
, SpeciesTypeNotValidAttribute          = 99918 /*!< Attribute 'speciesType' is not available on Species objects in SedML Level 1. */
, HasOnlySubsUnitsNotValidAttribute     = 99919 /*!< Attribute 'hasOnlySubstanceUnits' is not available on Species objects in SedML Level 1. */
, IdNotValidAttribute                   = 99920 /*!< Attribute 'id' is not available on SpeciesReference objects in SedML Level 1. */
, NameNotValidAttribute                 = 99921 /*!< Attribute 'name' is not available on SpeciesReference objects in SedML Level 1. */
, SpeciesTypeNotValidComponent          = 99922 /*!< The SpeciesType object is not supported in SedML Level 1. */
, StoichiometryMathNotValidComponent    = 99923 /*!< The StoichiometryMath object is not supported in SedML Level 1. */
, MultiplierNotValidAttribute           = 99924 /*!< Attribute 'multiplier' on Unit objects is not supported in SedML Level 1. */
, OffsetNotValidAttribute               = 99925 /*!< Attribute 'offset' on Unit objects is only available in SedML Level 2 Version 1. */
...
, PackageConversionNotSupported         = 99996 /*!< Conversion of SedML Level 3 package constructs is not yet supported. */
, InvalidTargetLevelVersion             = 99997 /*!< The requested SedML Level/Version combination is not known to exist. */
, L3NotSupported                        = 99998 /*!< SedML Level 3 is not yet supported. */
, SedMLCodesUpperBound                   = 99999
} SedMLErrorCode_t;


/**
 * Category codes for SedMLError diagnostics.
 *
 * Note that these are distinct from XMLError's category codes.  User
 * programs receiving an SedMLError object can use this distinction to
...
typedef enum 
{
  ...
  , LIBSEDML_CAT_SEDML_L1_COMPAT
     ...
     * translation from one Level/Version of SedML to another.  This
     * particular category applies to errors encountered while trying to
     * convert a model from SedML Level&nbsp;2 to SedML Level&nbsp;1. */

  , LIBSEDML_CAT_GENERAL_CONSISTENCY
    /*!< Category of errors that can occur while validating general SedML
     * constructs.  With respect to the SedML specification, these concern
  ...
  , LIBSEDML_CAT_IDENTIFIER_CONSISTENCY
     ...
     * identifiers in a model.  With respect to the SedML specification,
...
} SedMLErrorCategory_t;


/**
 * Severity codes for SedMLError diagnostics.
 *
...
typedef enum
{
  ...
  /* The following are used internally in SedMLErrorTable, but publicly,
   * we only report one of the 4 XMLError_Severity values.  Translation
   * of the codes is done in SedMLError.cpp.
   */
...
class LIBSEDML_EXTERN SedMLError : public XMLError
{
public:

  /**
   * Creates a new SedMLError to report that something occurred during SedML
   * processing.
   *
   * When a libSedML operation on SedML content results in a warning, error
   * or other diagnostic, the issue is reported as an SedMLError object.
   * SedMLError objects have identification numbers to indicate the nature
   * of the exception.  @if clike These numbers are drawn from
   * the enumeration <a class="el"
   * href="#SedMLErrorCode_t">
   * SedMLErrorCode_t</a>.  @endif@if java These numbers are
   * defined as unsigned integer constants in the file
   * "libsbmlConstants.html".  See the <a class="el"
   * href="#SedMLErrorCode_t">top of this documentation page</a> for a table
   ...
   * from <a class="el" href="#SedMLErrorCode_t">SedMLErrorCode_t</a>, the
   * SedMLError class assumes the error is a low-level system or SedML layer
   ...
   * href="#SedMLErrorCode_t">SedMLErrorCode_t</a> errors have associated
   * values for the @p severity and @p category codes, and these fields are
   * filled-in as well from the enumerations <a class="el"
   * href="#SedMLErrorSeverity_t">SedMLErrorSeverity_t</a> and <a class="el"
   * href="#SedMLErrorCategory_t">SedMLErrorCategory_t</a>,
   * respectively. @else set of constants.  If it @em
   * is one of the predefined error identifiers, the SedMLError class
   ...
   * SedMLError class assumes the error was generated from another part of
   * the software and does not do additional filling in of values beyond
   * the default in the constructor itself.  This allows SedMLError to serve
   ...
   * make maximum use of the SedMLError facilities.
   *
   * @if clike As mentioned above, there are two other
   * enumerations, <a class="el"
   * href="#SedMLErrorSeverity_t">SedMLErrorSeverity_t</a> and <a class="el"
   * href="#SedMLErrorCategory_t">SedMLErrorCategory_t</a>, used for indicating
   * the severity and category of error for the predefined SedMLError codes.
   ...
   * value from <a class="el" href="#SedMLErrorCode_t">SedMLErrorCode_t</a>,
   * callers do not need to fill in @p severity and @p category.
   * Conversely, if @p errorId is not a value from <a class="el"
   * href="#SedMLErrorCode_t">SedMLErrorCode_t</a>, callers can use other
   * values (not just those from <a class="el"
   * href="#SedMLErrorSeverity_t">SedMLErrorSeverity_t</a> and <a class="el"
   * href="#SedMLErrorCategory_t">SedMLErrorCategory_t</a>, but their own
   * special values) for @p severity and 
   * @p category. @else As mentioned above, 
   * there are additional constants defined for <a class="el"
   * href="#SedMLErrorSeverity_t">standard severity</a> and <a class="el"
   * href="#SedMLErrorCategory_t">standard category</a> codes, and every predefined 
   ...
   * @p category. @endif@~
   *
   * Please see the top of the documentation for SedMLError for a longer
   ...
   * belongs.
   *
   * @param package the SedML Level package involved.
   *
  ...
  /**
   * Copy constructor; creates a copy of this SedMLError.
   */
  SedMLError(const SedMLError& orig);
   ...
   * clone function
   */
  virtual SedMLError* clone() const; 

  /**
   * Outputs this SedMLError to stream in the following format (and followed
   * by a newline):
   *
